...
- (void)displayFullscreenImage:(UIImage *)image
{
    ...
    //-- Setup image view
    self.imageView = [[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, window.frame.size.width, window.frame.size.height)] autorelease];
    ...
    //-- Event handling
    UITapGestureRecognizer *tapRecognizer = [[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideImage)] autorelease];
